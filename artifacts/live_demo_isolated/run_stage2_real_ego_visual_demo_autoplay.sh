...
LAUNCH_CMD_PATTERN="roslaunch -p ${MASTER_PORT} human_follow_bringup stage2_real_ego_visual_demo_autoplay.launch"
...
LAUNCH_PID_FILE="$PID_DIR/stage2_real_ego_visual_demo_autoplay.pid"
LAUNCH_LOG="$LOG_DIR/stage2_real_ego_visual_demo_autoplay.log"
...
if launch_pid="$(find_running_pid "$LAUNCH_CMD_PATTERN")"; [[ -n "$launch_pid" ]] && kill -0 "$launch_pid" 2>/dev/null; then
  ...
  echo "demo autoplay launch already running pid=$launch_pid"
else
  setsid env \
    ...
    bash -lc "source /opt/ros/noetic/setup.bash && source '$ROOT/devel/setup.bash' && exec roslaunch -p ${MASTER_PORT} human_follow_bringup stage2_real_ego_visual_demo_autoplay.launch ${EXTRA_LAUNCH_ARGS[*]-}" \
