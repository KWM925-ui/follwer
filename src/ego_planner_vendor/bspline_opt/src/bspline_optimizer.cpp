...
namespace ego_planner
{
  namespace
  {
    double computeCheckedCurveLength(UniformBspline &traj, const double checked_duration_sec)
    {
      if (checked_duration_sec <= 0.0)
        return 0.0;

      const double sample_dt = std::max(std::min(checked_duration_sec / 200.0, 0.01), 1e-3);
      Eigen::VectorXd prev_point = traj.evaluateDeBoorT(0.0), current_point;
      double length_m = 0.0;
      for (double t_sec = sample_dt; t_sec <= checked_duration_sec + 1e-6; t_sec += sample_dt)
      {
        current_point = traj.evaluateDeBoorT(std::min(t_sec, checked_duration_sec));
        length_m += (current_point - prev_point).norm();
        prev_point = current_point;
      }
      return length_m;
    }

    double computeCollisionCheckStepSec(
        UniformBspline &traj,
        const double checked_duration_sec,
        const double spatial_resolution_m)
    {
      if (checked_duration_sec <= 0.0)
        return 1e-3;

      const double safe_resolution_m = std::max(spatial_resolution_m, 1e-3);
      const double max_arc_step_m = std::max(safe_resolution_m * 0.5, 1e-3);
      const double checked_length_m = computeCheckedCurveLength(traj, checked_duration_sec);
      if (checked_length_m <= 1e-6)
        return checked_duration_sec;

      const int sample_count = std::max(1, static_cast<int>(std::ceil(checked_length_m / max_arc_step_m)));
      return std::max(checked_duration_sec / static_cast<double>(sample_count), 1e-3);
    }
  } // namespace
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        const double checked_duration_sec = std::max((tmp - tm) * 2.0 / 3.0, 0.0);
        const double t_step = computeCollisionCheckStepSec(traj, checked_duration_sec, grid_map_->getResolution());
        for (double t = 0.0; t <= checked_duration_sec + 1e-6; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      traj.getTimeSpan(tm, tmp);
      const double checked_duration_sec = std::max((tmp - tm) * 2.0 / 3.0, 0.0);
      const double t_step = computeCollisionCheckStepSec(traj, checked_duration_sec, grid_map_->getResolution()); // Keep the recheck arc-length gap below half a voxel.
      for (double t = 0.0; t <= checked_duration_sec + 1e-6; t += t_step)
      {
