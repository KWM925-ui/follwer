...
namespace ego_planner
{
  ...
  void EGOReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    ROS_INFO_STREAM(
        "[ego waypoint] odom=" << odom_pos_.transpose()
                               << " end_pt=" << end_pt_.transpose()
                               << " success=" << (success ? 1 : 0)
                               << " global_duration=" << planner_manager_->global_data_.global_duration_);
  ...
  void EGOReplanFSM::getLocalTarget()
  {
    ...
    ROS_INFO_STREAM_THROTTLE(
        0.5,
        "[ego local_target] start=" << start_pt_.transpose()
                                     << " end_pt=" << end_pt_.transpose()
                                     << " local_target=" << local_target_pt_.transpose()
                                     << " last_progress=" << planner_manager_->global_data_.last_progress_time_
                                     << " global_duration=" << planner_manager_->global_data_.global_duration_
                                     << " horizon=" << planning_horizen_);
  }
