...
class TargetWorldProjectorNode:
    def __init__(self):
        ...
        self.output_topic = rospy.get_param("~output_topic", "/follow/fusion/target_world")
        ...
        self.target_sub = rospy.Subscriber(self.input_topic, Target3D, self._target_callback, queue_size=20)
        self.odom_sub = rospy.Subscriber(self.odom_topic, Odometry, self._odom_callback, queue_size=20)

        rospy.loginfo(
            "target_world_projector ready: input=%s odom=%s output=%s",
            self.input_topic,
            self.odom_topic,
            self.output_topic,
        )
...
if __name__ == "__main__":
    rospy.init_node("target_world_projector")
    TargetWorldProjectorNode()
    rospy.spin()
