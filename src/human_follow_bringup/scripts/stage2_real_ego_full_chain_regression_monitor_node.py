...
class Stage2RealEgoFullChainRegressionMonitorNode:
    def __init__(self):
        ...
        self.ego_waypoint_topic = rospy.get_param("~ego_waypoint_topic", "/waypoint_generator/waypoints")
        ...
        self.position_tolerance = float(rospy.get_param("~position_tolerance", 0.18))
        self.velocity_tolerance = float(rospy.get_param("~velocity_tolerance", 0.25))

        self.start_time = time.monotonic()
        self.last_goal = None
        self.last_ego_goal = None
        self.last_ego_waypoint = None
        ...
        rospy.Subscriber(self.ego_waypoint_topic, Path, self._ego_waypoint_callback, queue_size=20)
        ...
        rospy.loginfo(
            "stage2_real_ego_full_chain_regression_monitor ready goal=%s cmd=%s bridge=%s mavros=%s",
            self.goal_topic,
            self.ego_cmd_topic,
    ...
    def _fail(self, reason):
        ...
        self.finished = True
        self.exit_code = 1
        rospy.logerr("stage2 real ego full-chain regression FAIL %s", reason)
    ...
    def _pass(self, reason):
        ...
        self.finished = True
        self.exit_code = 0
        rospy.loginfo("stage2 real ego full-chain regression PASS %s", reason)
        rospy.signal_shutdown(reason)
    ...
    def _check_contracts(self):
        ...
        if self.last_ego_waypoint is None:
            return False, "waiting_waypoint_bridge"
        ...
        if not self.last_ego_waypoint.poses:
            return False, "empty_waypoint_path"
        ...
        waypoint_pose = self.last_ego_waypoint.poses[0].pose.position
        if (
            not self._close(goal.x, waypoint_pose.x, self.position_tolerance)
            or not self._close(goal.y, waypoint_pose.y, self.position_tolerance)
            or not self._close(goal.z, waypoint_pose.z, self.position_tolerance)
        ):
            return False, "goal_waypoint_bridge_mismatch"
...
if __name__ == "__main__":
    rospy.init_node("stage2_real_ego_full_chain_regression_monitor")
    Stage2RealEgoFullChainRegressionMonitorNode()
    rospy.spin()
