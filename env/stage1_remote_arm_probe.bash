...
roscore >/tmp/stage1_arm_probe_roscore.log 2>&1 &
ROSCORE_PID=\$!
...
roslaunch --wait human_follow_bringup stage1_usb_cam.launch video_device:='${VIDEO_DEVICE}' >/tmp/stage1_arm_probe_usb.log 2>&1 &
USB_PID=\$!
sleep 4

roslaunch --wait human_follow_bringup stage1_px4_mavros.launch fcu_url:='${FCU_URL}' gcs_url:='${GCS_URL}' >/tmp/stage1_arm_probe_mavros.log 2>&1 &
MAV_PID=\$!
...
rospy.init_node("stage1_arm_probe_connected_waiter", anonymous=True)
...
roslaunch --wait human_follow_bringup stage1_live_px4_to_mavros.launch >/tmp/stage1_arm_probe_follow.log 2>&1 &
FOLLOW_PID=\$!
...
rospy.init_node("stage1_arm_probe_setpoint_waiter", anonymous=True)
...
rospy.init_node("stage1_arm_probe_mode_waiter", anonymous=True)
rospy.Subscriber("/mavros/state", State, cb, queue_size=10)
...
rospy.init_node("stage1_arm_probe_armed_waiter", anonymous=True)
rospy.Subscriber("/mavros/state", State, cb, queue_size=10)
deadline = time.time() + 10.0
rate = rospy.Rate(20)
while time.time() < deadline and not rospy.is_shutdown():
    if seen["armed"]:
        ...
        sys.exit(0)
    rate.sleep()
print("[FAIL] armed state not observed last=%s" % seen["last"], file=sys.stderr)
sys.exit(1)
PY
...
rospy.init_node("stage1_arm_probe_disarmed_waiter", anonymous=True)
rospy.Subscriber("/mavros/state", State, cb, queue_size=10)
deadline = time.time() + 10.0
rate = rospy.Rate(20)
while time.time() < deadline and not rospy.is_shutdown():
    if seen["disarmed"]:
        print("[disarm_ok] False")
        sys.exit(0)
    rate.sleep()
print("[FAIL] disarmed state not observed last=%s" % seen["last"], file=sys.stderr)
sys.exit(1)
PY
EOF

echo "[PASS] remote arm probe passed"
